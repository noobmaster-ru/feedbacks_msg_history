...
class GoogleSheets:
    ...
    def update_time(self):
        ...
        try:
            sheetname = sh.worksheet(config.CHATS_SHEETNAME)
        except gspread.exceptions.WorksheetNotFound:
            sheetname = sh.add_worksheet(title=config.CHATS_SHEETNAME, rows=1000, cols=26)
        daytime = f"{datetime.now().strftime("%H:%M:%S")}"
        sheetname.update("G1", [["Время обновления", daytime]])
